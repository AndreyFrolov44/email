...
async def read_contacts(
    ...
    if list is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Данного списка не существует")
    if list.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="У вас нет доступа к данному списку")
...
async def create_contacts(
    ...
    for c in contact:
        ...
        if list is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=f"Списка {c.list_id} не существует")
        if list.user_id != current_user.id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail=f"У вас нет доступа к списку {c.list_id}")
